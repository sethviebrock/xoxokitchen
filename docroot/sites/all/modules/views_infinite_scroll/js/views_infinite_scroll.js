    ...
    if($.autopager) {
      if(!views_infinite_scroll_was_initialised) {
        ...
        if(Drupal.settings.views_infinite_scroll.length == 1) { 
          ...
          // alter target selector if we use js scroll pane plugin
          settings.content_selector = 'div.jspPane';
          var use_ajax = false;
          ...
          if(!use_ajax) {
            var content_selector = 'div.view-id-' + settings.view_name + '.view-display-id-' + settings.display + ' ' + settings.content_selector; // 'div.jspPane'
            // alter the source or data items if we use js scroll pane plugin
            var items_selector   = 'div.view-id-' + settings.view_name + '.view-display-id-' + settings.display + ' div.view-content .item-wrapper'; //content_selector + ' ' + settings.items_selector;
            ...
            $.autopager({
              ...
              load: function() {
                ...
                $('div.view-id-' + settings.view_name + '.view-display-id-' + settings.display + ' div.view-content').jScrollPane(); 
              }    
            });
          }
          else {  
